...
\infer[\textit{Left-Simplify-Rule}]
  ...
  {E, \ \  R \cup \{ s \to t \}} \\
\mbox{if $s \to_R u$ using a 
$l \to r \in R$ such that $s \sqsupset l$\rule[-6ex]{0ex}{-50ex}}
\end{array}
&
\begin{array}{ll}
\infer[\textit{Right-Simplify-Rule}]
  {\strut E, \ \  R \cup \{ s \to u \}}
  {E, \ \  R \cup \{ s \to t \}} \\
\mbox{if $t \to_R u$\rule[-3ex]{0ex}{6ex}}
\end{array}
\end{array}
\]
\end{small}
...
are due to
Bachmair, Dershowitz, and Hsiang~\cite{BachmairDershowitzHsiang1986LICS,Bachmair1991}.
An extension of completion to completion modulo
