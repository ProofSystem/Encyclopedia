...
\calculusName{HO Sequent Calculi $\SEQCALCD$ and $\SEQCALCFB$}   
\calculusAcronym{GBetaFB}     
\calculusLogic{Classical Logic}  
\calculusLogicOrder{Higher-Order}
\calculusType{Resolution and Superposition}   
\calculusYear{2003-2009}   

\calculusAuthor{Christoph Benzm{\"u}ller} 
\calculusAuthor{Chad Brown}
\calculusAuthor{Michael Kohlhase} 


\entryTitle{HO Sequent Calculi $\SEQCALCD$ and $\SEQCALCFB$}     
...
\end{calculus}
 ...
 \begin{clarifications}
   ...
   formulas of classical higher-order logic (HOL; Church's Type Theory)~\cite{sep-type-theory-church}. 
   $\Delta,s$ denotes the set
   $\Delta\cup\{s\}$. Let
...
%   Theorem proving in these calculi works as follows: 
In order to
   prove that a (closed) conjecture $c$ logically follows from
...
\end{clarifications}
 ...
 \begin{history}
These calculi were presented in~\cite{J18}, and
earlier (two-sided) related versions in~\cite{R19} and~\cite{BrownPhD}.
