...
\calculusName{Multi-type Sequent Calculi Mt.SC}
\calculusAcronym{MtSC}
\calculusLogic{Classical and non-classical, modal and substructural logics}
\calculusLogicOrder{Propositional}
\calculusType{Multi-type Sequent Calculus}
\calculusYear{2014, 2016}
\calculusAuthor{Giuseppe Greco} \calculusAuthor{Alessandra Palmigiano} \calculusAuthor{Alexander Kurz} \calculusAuthor{Sabine Frittella} \calculusAuthor{Vlasta Sikimi\'{c}} \calculusAuthor{Apostolos Tzimoulis} \calculusAuthor{Zhiguang Zhao} \calculusAuthor{Minghui Ma} 

\entryTitle{Multi-type Sequent Calculi Mt.SC}
...
\AXC{\fns LId}
\noLine
\UIC{$p^{\texttt{i}} \fCenter C^{\texttt{i}}[p^{\texttt{i}}]^{suc}$}
\DisplayProof 
\AXC{\fns RId}
\noLine
\UIC{$C^{\texttt{i}}[p^{\texttt{i}}]^{pre} \fCenter p^{\texttt{i}}$}
\DisplayProof 
\qquad
...
Cut elimination and subformula property for MtSC can be inferred from a meta-theorem, following the strategy introduced by Belnap for display calculi. These properties are preserved when adding or removing analytic structural rules. Logics which are not properly displayable have been presented as proper multi-type display calculi, see for instance semi De Morgan logic \iref{MtDSDM}, inquisitive logic \iref{MtDInqL}, lattice logic \iref{MtDLatL}, dynamic epistemic logic \iref{MtDDEL}, linear logic \cite{GrecoPalmigiano}, and propositional dynamic logic \cite{FrittellaGrecoKurzPalmigiano2016}. 
