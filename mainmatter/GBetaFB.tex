...
\calculusName{One-Sided Higher-Order Sequent Calculi $\SEQCALCD$ and $\SEQCALCFB$}   % The name of the calculus
...
\calculusYear{2004-2009}   % The year when the calculus was invented.
\calculusAuthor{Christoph Benzm{\"u}ller \and Chad Brown \and Michael Kohlhase} % The name(s) of the author(s) of the calculus.


\entryTitle{One-Sided Higher-Order Sequent Calculi $\SEQCALCD$ and $\SEQCALCFB$}     % Title of the entry (usually coincides with the name of the calculus).
...
Calculus $\SEQCALCD$ is defined by 
...
% do not look nice in a short entry.
 ...
 \begin{clarifications}
   ...
   formulas of classical higher-order logic (HOL)
   \cite{sep-type-theory-church} and let $\Delta,s$ denote the set
   $\Delta\cup\{s\}$. HOL terms and formulas: Let
   $\alpha,\beta,o \in {T}$. The \emph{terms} of HOL are defined by
   ...
   \emph{$\beta$-normal form} (denoted $\Bnormform{s}$) and a unique
   $\beta\eta$-normal form (denoted $\Benormform{s}$).  HOL formulas
...
Earlier versions and further related sequent calculi for HOL are presented in \cite{R19}.
...
\begin{technicalities} 
  $\SEQCALCD$ is sound and complete for elementary type theory (ETT):
  $\models^{\textrm{ETT}} c \textrm{ if and only if } \vdash^{\SEQCALCD}
  \{c\}$.
  $\SEQCALCD$ is thus also sound for HOL.  $\SEQCALCFB$ is sound and
  complete for HOL:
  $\models^{\textrm{HOL}} c \textrm{ if and only if } \vdash^{\SEQCALCFB}
  \{c\}$
\end{technicalities}
