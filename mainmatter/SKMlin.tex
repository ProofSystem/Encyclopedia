...
\calculusLogic{Superintuitionistic modal logic of linear Kripke frames}
\calculusType{Sequent Calculus} 
\calculusYear{2014} 
\calculusAuthor{Ranald Clouston \and Rajeev Gor\'e}
...
\begin{clarifications}
  ...
  one also converts $\varphi\limp\psi$ to $\varphi\iimp\psi$ read
  upwards building in a form of contraction on such formulae. The
  $\steprulename$ rule has an indeterminate number of premises, one
  for each $\varphi_i\iimp\psi_i \in \opset{\iimp}{\Delta}$, and one
  for each $\varphi_i \in \opset{\nxt}{\Phi}$. For each such
  ``eventuality'', the rule creates a premise that contains the
  subformula on an appropriate side, but also creates a copy of the
  principal formula in the antecedent of that premise, thus building
  in aspects of the standard sequent calculus for G\"oedel-L\"ob
  logic.
\end{clarifications}

\begin{history}
%
  % Guarded recursion~\cite{Coquand:Infinite} on an infinite data
  % structure requires that recursive calls be nested beneath
  % constructors. Nakano~\cite{Nakano:Modality} showed that guarded
  % recursion can be enforced through the \emph{type system} via an
  % `approximation modality' inspired by G\"odel-L\"ob provability
  % logic~\cite{boolos-provability-logic}. This modality is called
  % \emph{later}, and we write it using the symbol $\nxt$. The meaning
...
sequent calculus for G\"odel-Dummett logic
LC~\cite{DBLP:journals/sLogica/Corsi92}\footnote{Need to confirm.};
and George
...
\begin{technicalities}
  ...
  is coNP-complete. The sequent calculus can be used for
  backtrack-free and terminating backward proof search via the
  following strategy for rule applications: apply any applicable rule
  backwards, always preferring zero-premise rules if possible!
...
% \hphantom{11}\textbf{transition:} else apply the transitional rule to
% that leaf
